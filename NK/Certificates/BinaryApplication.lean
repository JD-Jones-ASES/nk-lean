...
natural powers of a choice-based depth, whose exact value is proved separately.
This prevents eager independent-kernel reduction to enormous numerals.
-/
...
theorem binary_alphabet_exists :
    ∃ A : IntervalAlphabet 2 (4 ^ binaryDepth),
      (∀ x (hx : x ∈ A.support), (A.interval x hx).width < 1) ∧
      (((4 ^ binaryDepth : ℕ) : ℝ) ^ (75806770413 / 100000000000 : ℝ)) ≤
  ...
  obtain ⟨A, hwidth, hmoment⟩ := binaryPolicy.exists_half_depth
    ...
    (fun i => binaryRows_growth i) binaryDepth binaryDepth_ge_one 1
...
noncomputable def binary_component : MomentComponent 2 (75806770413 / 100000000000 : ℝ) where
  ...
  base_ge_two := le_trans (by decide : 2 ≤ (4 : ℕ))
    (le_self_pow (by decide : 1 ≤ (4 : ℕ)) (Nat.ne_of_gt binaryDepth_pos))
  base_eq_power := by
...
theorem binary_component_base : binary_component.base = 4 ^ binaryDepth := rfl

theorem binary_component_root : binary_component.root = 2 ^ binaryDepth := rfl
